...
$(document).on 'click', '.logout-credit-card', (e) ->
  setStripeToken( $(this).parents('form.logout_form:first') )
        
setStripeToken = ( container ) ->
  $container = $(container)
  if $('.card-bg').is(':visible') || $('.add-card-wrapper').hasClass('show')
    debugger
    if($container.find('#service_request_token').val().length == 0)
...
stripeResponseHandler = (status, response) ->
  $form = $('form#add_new_card, form#service-request-form, form.logout-out')
  if response.error
    ...
    if !$('form').hasClass('logout-form')
      ...
      $('.content-wrapper.signup-form').removeClass('hidden')
